...
public struct VecByte4(byte x, byte y, byte z, byte w)
{
	public byte X = x;
	public byte Y = y;
	public byte Z = z;
...
public class Model
{
	...
	public Model(string path)
	{
		...
		foreach (var primitive in gltf_model.LogicalMeshes[0].Primitives)
		{
			...
			for (int i = 0; i < verts.Count; i++) vertices.Add(new MeshVertex(verts[i], uvs[i], normals[i], colors != null ? colors[i] : Color.White, VecByte4.Zero));
			foreach (var index in primitive.GetIndices()) indices.Add((int)index);
		}
